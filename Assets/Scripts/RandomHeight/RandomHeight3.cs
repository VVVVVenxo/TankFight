...
public class RandomHeight3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (t2 - t1 >= spawnInterval && GameObject.FindGameObjectsWithTag("Circle").Length < maxCubes)
        {
            x = Random.Range(-35f, 35f); // 指定 x 轴方向上的范围
            z = Random.Range(-35f, 35f); // 指定 z 轴方向上的范围
            Instantiate(cube, new Vector3(x, 0.5f, z), Quaternion.Euler(-90,0,0)); // 随机生成物体（预制体，生成的位置，方向）。
